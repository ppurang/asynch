...
object ThreadFactoryCreator {
  ...
  def default[F[_]](implicit S: Sync[F]): F[ThreadFactoryCreator[F]] = S.delay {
    new ThreadFactoryCreator[F] {
      override def threadFactory(prefix: String,
                                 threadType: ThreadType,
                                 priority: ThreadPriority,
                                 uncaughtExceptionHandler: Thread.UncaughtExceptionHandler): F[ThreadFactory] = S.delay {
        val dtf = new DefaultThreadFactory(
          prefix, threadType.isDaemon, priority.javaThreadPriority, uncaughtExceptionHandler
        )
        //fail fast
        ...
        require(t.getName.contains(prefix), s"thread factory created a thread '${t.getName}' without the suggested $prefix")
        dtf
      }
    }
  }
